...
def exercise0():
    ...
    print("synsets order with respect to shortest path from right_whale.n.01 is : ", sort_synset(wn_synsets, 'right_whale.n.01'))
...
def exercise26():
    ...
    for i in range(stop):
        ...
        ## un comment this to see the real time graph for these values
...
def sort_on_documentorder(result):
...
def get_sentences_score(n):
    ...
    for sentence in sentences:
        ...
        sentence_lookup[sentence] = (sum([lookup[word] for word in words_in_sentence])/len(words_in_sentence), count)
        count += 1
    result = sorted(sentence_lookup.items(), key=lambda x: x[1][0])[::-1]
    return sort_on_documentorder(result[:n])


def exercise32():
    tests = [5, 7, 10, 15, 20, 30]
    for n in tests:
        print("N value : ", n)
        print(get_sentences_score(n))
